...
void meteorRain(byte red, byte green, byte blue, byte meteorSize, byte meteorTrailDecay, boolean meteorRandomDecay, int SpeedDelay)
{
  ...
  for (int i = 0; i < NUM_LEDS; i++) // Start from the first LED and go forwards
  {
    ...
    {
      if ((i + j < NUM_LEDS)) // Adjust to ensure it doesn't go out of bounds
      {
...
void loop()
{
  if ((int(incomingDat2) % 2) == 1)
  {
    meteorRain(0xff, 0x5e, 0x00, 1, 84, true, 0);
  }
  else
  {
    meteorRain(0xff, 0xff, 0xff, 1, 84, true, 0);
  }
  FastLED.clear();
  FastLED.show();
}